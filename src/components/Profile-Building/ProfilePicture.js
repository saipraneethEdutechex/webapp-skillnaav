...
const ProfilePictureForm = ({ onSubmit }) => {
  ...
  const isFormValid = () => {
    const { fieldOfStudy, profilePicture, resume, linkedin, portfolio } = formData;
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100 font-poppins">
      <div className="w-full max-w-xl p-8 space-y-6 bg-white shadow-md rounded-lg">
        ...
        <div className="space-y-4">
          <div className="w-full h-12 p-3 bg-purple-100 border-b border-purple-300">
            <h2 className="text-lg font-bold text-gray-700">LINKS AND DOCUMENTS</h2>
          </div>
            ...
            <div className="flex flex-col items-center">
              ...
              {preview ? (
                <div className="relative">
                  <img src={preview} alt="Profile Preview" className="w-16 h-16 object-cover rounded-full" />
                  <button
                    type="button"
                ...
                >
                  <img src={uploadButtonIcon} alt="Choose File" className="w-16 h-16 object-cover" />
                </label>
              )}
            </div>
          </div>
          <div className="flex flex-col items-center space-y-2">
            <label className="block text-sm font-medium text-gray-700">Resume or CV</label>
            <div className="flex flex-col items-center">
              <input
                type="file"
                name="resume"
              ...
              {formData.resume ? (
                <div className="relative">
                  <span className="block w-16 h-16 text-center leading-16">{formData.resume.name}</span>
                  <button
                    type="button"
        ...
        <div>
          <button
            ...
            className={`w-full py-2 px-4 border rounded-md shadow-sm text-sm font-medium ${buttonDisabled ? 'bg-[#E9D7FE] text-[#B4A0D1]' : 'bg-purple-600 text-white hover:bg-purple-700'}`}
